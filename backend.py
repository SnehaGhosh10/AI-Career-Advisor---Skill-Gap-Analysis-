...
Now perform:
1. Extract candidate's name, technical skills, experience (roles + duration), and education.
2. Extract relevant skills, qualifications, and responsibilities from the JD.
3. Perform a skill gap analysis, listing only missing or weak skills relevant to the JD.
4. Suggest 2-3 concise, practical resume improvements to align with the JD.
5. Give short, actionable personalized career advice.
...
Output only the JSON, without markdown, commentary, or explanations.
"""
...
def analyze_resume_vs_jd(uploaded_file, jd_text):
    ...
    # Try JSON extraction with regex
    try:
    ...
    # Fallback using ast.literal_eval if LLM returned Python dict
    try:
